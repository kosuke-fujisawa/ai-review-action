...
- 構造化プローブ(`git_ls_files`)は任意の補助証拠です。差分・実行経路・反証結果だけで確信度highと判断できる指摘はプローブなしでも採用します。指摘と無関係なプローブ結果は証拠として扱わず、関連するプローブが指摘と矛盾する結果を返した場合はその指摘を棄却します
...
現在許可している構造化プローブは `git_ls_files` のみです。`--cached` と最大10個のpathspecだけを受け付け、未ステージ追跡ファイルが列挙されるかを実結果と照合します。検証対象がない場合や指摘と無関係な場合は、確認済みとして扱わず、コメントに「機械検証なし」または「本指摘とは無関係のため未使用」と明記します。
